...
} from '@digitalshieldfe/digitalshield-near-provider';
...
declare global {
  interface Window {
    provider: DigitalShieldNearProvider;
    nearAPI: typeof NearApi;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    DigitalShieldNearProvider: any;
  }
}

window.nearAPI = NearApi;
window.DigitalShieldNearProvider = DigitalShieldNearProvider;
...
export default function NearExample() {
  const [provider, setProvider] = useState<DigitalShieldNearProvider | null>(null);
  ...
  useEffect(() => {
    ...
    const _provider = new DigitalShieldNearProvider({
      // connection,
  ...
  return (
    <div>
      ...
      {!provider && (
        <a target="_blank" href={'https://www.digitalshield.so/download/'}>
          Install DigitalShield Extension →
        </a>
      )}
      {provider && (
        <div>
            ...
            <button
              onClick={async () => {
                const res = (await provider.request({
                  ...
                  params: {
                    messages: ['hello world', 'digitalshield near wallet'],
                  },
                })) as SignMessagesResult;
