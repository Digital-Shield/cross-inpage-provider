...
type DigitalShieldTronProviderProps = IInpageProviderConfig & {
  timeout?: number;
};
...
export const TRON_REQUEST_ACCOUNTS_LOCAL_KEY = 'digitalshield_tron_request_accounts_local_key';
...
class DigitalShieldTronWeb extends TronWeb {
...
class ProviderTron extends ProviderTronBase implements IProviderTron {
  ...
  constructor(props: DigitalShieldTronProviderProps) {
    super({
      ...props,
  ...
  private _registerTronWeb(nodes: Nodes): { tronWeb: TronWeb; sunWeb: SunWeb } | null {
    ...
    const tronWeb: TronWeb = new DigitalShieldTronWeb(
      {
        ...nodes,
      },
      this,
    ...
    const tronWeb1: TronWeb = new DigitalShieldTronWeb(
      {
        ...nodes,
      },
      this,
    ...
    const tronWeb2: TronWeb = new DigitalShieldTronWeb(
      {
        ...nodes,
      },
      this,
  ...
  private async _initialize() {
    try {
      ...
      if (window.tronWeb !== undefined) {
        this._log.warn(
          'DigitalShield: TronWeb is already initiated. Digitalshield will overwrite the current instance',
        );
      }
      ...
      Object.defineProperty(tronWeb, 'defaultAddress', {
        get() {
          if (!self._connected) {
            self._log.warn(
              'DigitalShield: We recommend that DApp developers use $digitalshield.tron.request({method: "tron_requestAccounts"}) to request users’ account information at the earliest time possible in order to get a complete TronWeb injection.',
            );
      ...
      this._log.error('DigitalShield: Failed to get initial state. Please report this bug.', error);
    }
  }
  ...
  private _handleAccountsChanged(accounts: string[]) {
    ...
    if (!Array.isArray(accounts)) {
      this._log.error(
        'Digitalshield: Received invalid accounts parameter. Please report this bug.',
        accounts,
      );
      _accounts = [];
    }
    ...
    for (const account of _accounts) {
      if (typeof account !== 'string') {
        this._log.error('Digitalshield: Received non-string account. Please report this bug.', accounts);
        _accounts = [];
        break;
      }
    }
