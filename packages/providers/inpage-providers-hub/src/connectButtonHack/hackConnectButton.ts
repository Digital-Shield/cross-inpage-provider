...
function checkIfInjectedProviderConnected({
  ...
  const hub = window.$digitalshield as IWindowDigitalShieldHub;
...
export async function detectQrcodeFromSvg({
  ...
  const res = (await (window.$digitalshield as IWindowDigitalShieldHub)?.$private?.request({
...
function addRotateAnimationToCss() {
  ...
  css.insertRule(
    `
@keyframes digitalShieldSpinner {
  0% {
...
export function createWalletConnectToButton({
  ...
  const datasetKey = 'digitalshield_auto_created_wallet_connect_btn'; // can not include `-`
...
text-align: center;
    `;
    // i18n key:
    //    action__connect_digitalshield_extension
    //    action__connect_digitalshield
    btn.innerHTML = `
    <span>Connect DigitalShield</span>
    <span class='digitalshield-spinner-element' style='
    display: none;
    ...
    btn.onclick = () => {
      ...
      void digitalshieldHub?.$private?.request({
        method: 'wallet_connectToWalletConnect',
        params: { uri },
      });
      const spinner = btn.querySelector('.digitalshield-spinner-element') as HTMLElement | undefined;
      if (spinner) {
        spinner.style.animation = 'digitalShieldSpinner 1s linear infinite';
...
export function createNewImageToContainer({
  ...
  const datasetKey = 'digitalshield_auto_created_icon_img'; // can not include `-`
  if (!container.querySelector(`[data-${datasetKey}]`)) {
