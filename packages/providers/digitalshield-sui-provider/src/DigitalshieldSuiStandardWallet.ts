...
import { hexToBytes } from '@digitalshieldfe/cross-inpage-provider-core';

import { ALL_PERMISSION_TYPES } from './types';

import type { ProviderSui } from './DigitalshieldSuiProvider';
...
class DigitalshieldSuiStandardWallet implements Wallet {
  readonly version = '1.0.0' as const;
  readonly _name = 'DigitalShield Wallet' as const;
...
export function registerSuiWallet(provider: ProviderSui, options?: WalletInfo) {
  try {
    registerWallet(new DigitalshieldSuiStandardWallet(provider, options));
  } catch (error) {
    console.error(error);
  }
}
