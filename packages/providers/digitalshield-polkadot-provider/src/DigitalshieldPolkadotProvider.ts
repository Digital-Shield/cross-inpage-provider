...
import DigitalShieldInjected from './inject/Injected';
...
export type DigitalShieldPolkadotProviderProps = IInpageProviderConfig & {
  timeout?: number;
};
...
class ProviderPolkadot extends ProviderPolkadotBase implements IProviderPolkadot {
  ...
  constructor(props: DigitalShieldPolkadotProviderProps) {
    super({
      ...props,
  ...
  private createMessage(payload: IMessagePayload): IPostMessage {
    return {
      id: 2,
      origin: 'DigitalShield Polkadot Provider',
      ...payload,
    };
  }
...
const registerPolkadot = (provider: ProviderPolkadot, name = 'DigitalShield', version = '1.0.0') => {
  try {
    const enableFn = async (originName: string): Promise<Injected> => {
      await provider.web3Enable(originName);
      return new DigitalShieldInjected(provider);
    };

    injectExtension(enableFn, { name: name ?? 'DigitalShield', version: version ?? '1.0.0' });
  } catch (error) {
    console.error(error);
  }
};
