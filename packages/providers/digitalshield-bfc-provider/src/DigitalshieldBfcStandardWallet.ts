...
import { ProviderBfc } from './DigitalshieldBfcProvider';
...
class DigitalshieldBfcStandardWallet implements Wallet {
...
export function registerBfcWallet(provider: ProviderBfc, options?: WalletInfo) {
  try {
    registerWallet(new DigitalshieldBfcStandardWallet(provider, options));
  } catch (error) {
    console.error(error);
  }
}
