...
import { IInpageProviderConfig, ProviderBase, switchDefaultWalletNotification, switchNetworkNotification } from '@digitalshieldfe/cross-inpage-provider-core';
import { consts } from '@digitalshieldfe/cross-inpage-provider-core';

export interface IDigitalShieldWalletInfo {
...
class ProviderPrivate extends ProviderBase {
        ...
        if (method === METHODS.wallet_events_ext_switch_changed) {
          ...
          if (!walletInfoLocal || (walletInfoLocal && walletInfoLocal.platformEnv.isExtension)) {
            try {
              ...
              this.notifyDefaultWalletChanged(params as IDigitalShieldWalletInfo)
            } catch (e) {
              console.error(e);
            }
          }
  ...
  notifyDefaultWalletChanged(params: IDigitalShieldWalletInfo) {
