...
type DigitalShieldConfluxProviderProps = IInpageProviderConfig & {
  timeout?: number;
};
...
class ProviderConflux extends ProviderConfluxBase implements IProviderConflux {
  ...
  public readonly isDigitalShield = true;
  ...
  constructor(props: DigitalShieldConfluxProviderProps) {
    super({
      ...props,
  ...
  private _handleChainChanged({ chainId, networkId }: Network = {}) {
    if (
      ...
      this._log.error('Digitalshield: Received invalid network parameters. Please report this bug.', {
        chainId,
        networkId,
      });
      return;
