...
import {DigitalShieldSolanaWalletAccount} from './account';
...
export const DigitalShieldNamespace = 'digitalshield:';

export type DigitalShieldFeature = {
    [DigitalShieldNamespace]: {
        digitalshield: ProviderSolana;
    };
};


export class DigitalShieldSolanaStandardWallet implements Wallet {
  readonly #listeners: { [E in StandardEventsNames]?: StandardEventsListeners[E][] } = {};
  readonly #version = '1.0.0' as const;
  readonly #name = 'DigitalShield' as const;
  readonly #icon: WalletIcon;
  #account: DigitalShieldSolanaWalletAccount | null = null;
  ...
  constructor(provider: ProviderSolana, options: WalletInfo) {
      if (new.target === DigitalShieldSolanaStandardWallet) {
          Object.freeze(this);
      }
  ...
  #connected = () => {
      ...
      if (address) {
          ...
          if (!account || account.address !== address || !bytesEqual(account.publicKey, publicKey)) {
              this.#account = new DigitalShieldSolanaWalletAccount({ address, publicKey });
