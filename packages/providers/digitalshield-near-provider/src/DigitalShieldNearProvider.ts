...
import { IJsonRpcRequest } from '@digitalshieldfe/cross-inpage-provider-types';
import { IInpageProviderConfig } from '@digitalshieldfe/cross-inpage-provider-core';
import { getOrCreateExtInjectedJsBridge } from '@digitalshieldfe/extension-bridge-injected';
import { web3Errors } from '@digitalshieldfe/cross-inpage-provider-errors';
...
export type DigitalShieldNearWalletProps = {
...
export type DigitalShieldWalletAccountProps = {
  wallet: DigitalShieldNearProvider;
  connection: unknown;
  accountId: string;
};
...
type DigitalShieldNearProviderEventsMap = {
...
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
declare interface DigitalShieldNearProvider {
  // TODO add request type define
  on<U extends keyof DigitalShieldNearProviderEventsMap>(
    event: U,
    listener: DigitalShieldNearProviderEventsMap[U],
    context?: any,
  ): this;
  emit<U extends keyof DigitalShieldNearProviderEventsMap>(
    event: U,
    ...args: Parameters<DigitalShieldNearProviderEventsMap[U]>
  ): boolean;
}
...
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
class DigitalShieldNearProvider extends ProviderNearBase {
  _enablePageReload?: boolean = false;
  _connectEagerly?: boolean = true;
  _authData: NearAccountInfo = DEFAULT_AUTH_DATA;
  _authDataKey = '@DigitalShieldNearWalletAuthData';
  _account?: DigitalShieldWalletAccount | null;
  ...
  constructor({
    ...
    if (!networkId) {
      // throw new Error('DigitalShieldNearWallet init error: networkId required.');
    }
  ...
  _registerEvents() {
    window.addEventListener('digitalshield_bridge_disconnect', () => {
  ...
  account() {
    ...
    if (!this._account || this._account.accountId !== accountId) {
      this._account = new DigitalShieldWalletAccount({
        wallet: this,
...
class DigitalShieldWalletAccount extends Account {
  _wallet: DigitalShieldNearProvider;

  constructor({ wallet, connection, accountId }: DigitalShieldWalletAccountProps) {
...
export { DigitalShieldNearProvider, DigitalShieldWalletAccount, serializeTransaction };
