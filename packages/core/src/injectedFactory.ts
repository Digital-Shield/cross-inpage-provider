...
function createCodeWithScriptTag({ code }: { code: string }): string {
  ...
  return `
    (function(){
      ...
      s.setAttribute('data-digitalshield-injected', 'true');
...
function injectCodeWithScriptTag({
  ...
  (function () {
    ...
    s.setAttribute('data-digitalshield-injected', 'yes');
    if (code) {
      s.textContent = code;
    }
    if (file) {
...
function createCodeJsBridgeReceive(payloadStr: string): string {
  return `
  if(window.$digitalshield && window.$digitalshield.jsBridge){
    window.$digitalshield.jsBridge.receive(${JSON.stringify(payloadStr)});
  }
  void 0;
  `;
}
