...
function customLocalStorage() {
  try {
    if (typeof global !== 'undefined' && global.$$digitalshieldAppStorage) {
      return global.$$digitalshieldAppStorage;
    }
    ...
    console.warn(
      'debugLogger init warning, neither `global.$$digitalshieldAppStorage` nor `window.localStorage` found.',
    );
    return {
      async getItem() {
        return '';
