...
import { IJsBridgeMessagePayload, IJsonRpcRequest } from '@digitalshieldfe/cross-inpage-provider-types';
...
function injectedProviderReceiveHandler(payload: IJsBridgeMessagePayload, bridge?: JsBridgeBase) {
  ...
  const providerHub = bridge?.providersHub || window.$digitalshield;
