...
import { fakeDebugLogger, consts } from '@digitalshieldfe/cross-inpage-provider-core';
  ...
  window.addEventListener(
    ...
    (event: MessageEvent) => {
      ...
      if (
        ...
        const jsBridge = options.bridge ?? (window?.$digitalshield?.jsBridge as JsBridgeBase);
        if (jsBridge) {
