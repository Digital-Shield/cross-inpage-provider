...
import { JsBridgeBase, consts } from '@digitalshieldfe/cross-inpage-provider-core';

const { EXT_PORT_OFFSCREEN_TO_BG, EXT_PORT_CS_TO_BG, EXT_PORT_UI_TO_BG, DIGITALSHIELD_REQUEST_TO_ALL_CS } = consts;
  ...
  requestToAllCS(scope: IInjectedProviderNamesStrings, data: unknown, targetOrigin?: string) {
    ...
    entries(this.ports).forEach(async ([portId, port]) => {
      if (port.name === EXT_PORT_CS_TO_BG) {
        ...
        if (!targetOrigin || targetOrigin === origin || targetOrigin === DIGITALSHIELD_REQUEST_TO_ALL_CS) {
